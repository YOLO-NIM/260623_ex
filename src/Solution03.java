...
class Calculator{
    ...
    int add(int a, int b){
        // 타입 지정 시 해당 타입을 반환하는 return이 꼭 있어야 함
//        int result = a + b;
//        return result;
        return a + b;
    }
    ...
    int add(int a, int b, int c){
        ...
        System.out.println("1"); // 매개변수 조합에 따라 다른 이름을 쓰지 않고도 같은 메서드 이름으로 호출하여 사용할 수 있게 // over'loading' - 과적
        return a + b + c;
    }

    // 패러미터 이름만 다르고 갯수, 타입이 중복되므로 컴파일 과정에서는 같은 걸로 침 (오버로딩 불가)
//    int add(int c, int d){
//        return c + d;
//    }

    // 가변 인자를 쓸 수 있음 - 대신 맨 뒤에 와야한다
